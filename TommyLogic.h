...
namespace TommyLogic {
	...
	vector<vector<int>> generateTruthtableYX(int n) {
		...
		for (unsigned col = 0; col < (size_t)n; ++col, num_to_fill >>= 1U)
		{
		...
		for (unsigned x = 0; x < (size_t)(1 << n); ++x)
		{
			for (unsigned y = 0; y < (size_t)n; ++y)
			{
				//std::cout << output[y][x] << " ";
			}
			//std::cout << std::endl;
	...
	void loopInputUseGate(UseGate obj, vector<vector<int>> input, vector<LogicGate*> OtherGatesResults) { //Without Circular Dependency and without timed output
		for (int i = 0; i < input.size(); i++) {
			obj.inputAll(input[i]);
			obj.call();
			cout << "Input: ";
